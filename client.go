...
func (client *Client) Go(serviceMethod string, args, reply any) *Call {
	call := &Call{
...
func (client *Client) Call(ctx context.Context, serviceMethod string, args, reply any) error {
	callChan := client.Go(serviceMethod, args, reply)
	select {
	case <-ctx.Done():
		client.removeCall(callChan.Seq)
		return errors.New("rpc client: call failed: " + ctx.Err().Error())
	case call := <-callChan.Done:
		return call.Error
	}
}
