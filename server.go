...
func (s *Server) Register(rvcv any) error {
	service := newService(rvcv)
	if _, exist := s.serviceMap.LoadOrStore(service.name, service); exist {
...
func Register(rvcv any) error { return DefaultServer.Register(rvcv) }
